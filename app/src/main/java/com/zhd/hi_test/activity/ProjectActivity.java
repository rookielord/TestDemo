...
public class ProjectActivity extends Activity {
    ...
    private void showProjectInfo(MyProject project) {
        tv_name.setText(project.getName());
        tv_coordinate.setText(project.getCoordinate());
        tv_guass.setText(project.getGuass());
        tv_lasttime.setText(project.getLastTime());
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (id) {
            case R.id.item_create:
                ...
                View view = inflater.inflate(R.layout.project_dialog, null);
                //填充坐标系和高斯投影带数据
        ...
        switch (item.getItemId()) {
            case 0:
                ...
                if (Const.getProject() != null) {
                    FileUtil.updateProject(Const.getProject());
                }
            ...
            case 1:
                ...
                break;
            case 2:
                View v=inflater.inflate(R.layout.project_detail_item, null);
                TextView tv_name,tv_coor,tv_guass,tv_lasttime,tv_back,tv_create;
                tv_name= (TextView) v.findViewById(R.id.tv_name);
                tv_guass= (TextView) v.findViewById(R.id.tv_guass);
                tv_coor= (TextView) v.findViewById(R.id.tv_coordinate);
                tv_lasttime= (TextView) v.findViewById(R.id.tv_lasttime);
                tv_back= (TextView) v.findViewById(R.id.tv_back);
                tv_create= (TextView) v.findViewById(R.id.tv_create_time);
                tv_name.setText(mProject.getName());
                tv_guass.setText(mProject.getGuass());
                tv_coor.setText(mProject.getCoordinate());
                tv_lasttime.setText(mProject.getLastTime());
                tv_back.setText(mProject.getBackup());
                tv_create.setText(mProject.getTime());
                AlertDialog dialog = new AlertDialog.Builder(this)
                        .setView(v)
                        .setTitle(R.string.project_show).create();
                dialog.show();
                break;
        }
    ...
    private void deleteProject() {
        ...
        //如果名称相同则当前项目为空
        if (Const.getProject() != null) {
            if (Const.getProject().getName().equals(mProject.getName())) {
        ...
        if (p != null) { //如果当前删除项目是最后一个打开项目
            if (p.getName().equals(mProject.getName())) {
                File file = new File(this.getFilesDir(), "last.txt");
                file.delete();
            }
        }
        //删除表
        Curd curd = new Curd(mProject.getTableName(), this);
        curd.dropTable(mProject.getTableName());
        //删除项目文件
        File file = new File(mPath + "/" + mProject.getName());
    ...
    public void refresh() {
        ...
        int loc = FileUtil.getDefaultProLoc(mProjects);
        mProjects.remove(loc);
        mProjects.add(0, Const.getProject());
        mpa.setmProjects(mProjects);
        showProjectInfo(Const.getProject());
        mProject = Const.getProject();
    }
