...
public class AddPointActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (id == null) {
            ...
            et_height.setText(String.valueOf(Const.Height));
            //获得最后的插入id，然后拼接成点号
            ...
            tv_add.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    values.add(cv);
                    //这里将mheight重新赋值给全局变量
                    Const.Height = (Float.parseFloat(et_height.getText().toString()));
            ...
            cursor.close();
            //获得原来的高度
            mheight = Float.valueOf(et_height.getText().toString());
            //将获取到的数据进行更新
            tv_add.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    ...
                    cv.put("DES", et_des.getText().toString());
                    cv.put("height", et_height.getText().toString());
                    cv.put("Z", updaetHeight(tv_Z.getText().toString()) + "m");
                    cv.put("H", updaetHeight(tv_H.getText().toString()) + "m");
    ...
    private String updaetHeight(String value) {
        float height = Float.valueOf(et_height.getText().toString());
        float offset = height - mheight;
        float H = Float.valueOf(value.substring(0, value.length() - 1));
        return String.valueOf(H + offset);
    }

    private String setHeight(String height) {
        float h = Float.parseFloat(height.substring(0, height.length() - 1));
        return String.valueOf(h + Const.Height);
    }
}
