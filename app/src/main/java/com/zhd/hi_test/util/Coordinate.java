...
public class Coordinate {
     ...
     * @return
     */
    public static double getDegreeFromSQL(String ddmmss) {
        String[] value = ddmmss.split(":");
        char Dire=value[2].charAt(value[2].length() - 1);
        double ss=Double.valueOf(value[2].substring(0,value[2].length()-2));
        double res=Integer.valueOf(value[0]) + Double.valueOf(value[1]) / 60 + ss / 3600;
        if (Dire=='N'|| Dire=='E')
            return res;
        else
            return -res;
     ...
     */
    public static HashMap<String, Double> getCoordinateNE(double latitude, double longtitude, MyProject project) {
...
//        高斯投影的中央经度
        ...
        info.put("n", Double.valueOf(saveAfterPoint(x, 5)));
        info.put("e", Double.valueOf(saveAfterPoint(y, 5)));
        return info;
    }

    private static void getConvertValue(MyProject myProject) {
        switch (myProject.getCoordinate()) {
            case "国家2000坐标系":
                A = 6378137.0;
                B = 6356752.31414;
                break;
