...
public class SurveyView extends View {
    ...
    private CopyOnWriteArrayList<MyPoint> mPoints = new CopyOnWriteArrayList<>();
    //需要画的点集合
    //如果用ArrayList会造成线程不安全
    private CopyOnWriteArrayList<DrawPoint> mDrawPoints = new CopyOnWriteArrayList<>();
    //我的位置
    ...
    public void SetCurrentLocation(MyPoint point) {
        if (mPoints.size() > 0) {
            mOffsetx = (float) (mREFpoint.getN() - point.getN()) * mScale;
            mOffsety = (float) (mREFpoint.getE() - point.getE()) * mScale;
        }
    }
    ...
    private void updatePoints() {
        ...
        if (mPoint != null) {
            if (mPoints.size() == 0) {//如果没有参考点的情况，即没有已知点的情况
                mDrawPoint = new DrawPoint(mCenterX, mCenterY);
            } else {//根据根据传进来的当前位置，获得在屏幕上的对应坐标
                float myX = (float) (mCenterX + (mPoint.getN() - mREFpoint.getN()) * mScale) + mOffsetx;
                float myY = (float) (mCenterY + (mPoint.getE() - mREFpoint.getE()) * mScale) + mOffsety;
                mDrawPoint = new DrawPoint(myX, myY);
            }
        }
        ...
        for (MyPoint myPoint : mPoints) {
            //要在自定义控件上画图的集合
            float x = (float) (mCenterX + (myPoint.getN() - mREFpoint.getN()) * mScale) + mOffsetx;
            float y = (float) (mCenterY + (myPoint.getE() - mREFpoint.getE()) * mScale) + mOffsety;
            DrawPoint p = new DrawPoint(x, y, myPoint.getName());
            mDrawPoints.add(p);
        }
        //4.获得最后一个点在屏幕上的坐标,这个有时候怎么会获取不到
        mLastPont = mDrawPoints.get(0);
    }
    ...
    private void DrawMypoint(Canvas canvas) {
        ...
        if (mDrawPoint != null) {
            canvas.drawCircle(mDrawPoint.getX(), mDrawPoint.getY(), 5, mPaint);
            mPaint.setColor(Color.BLACK);
            if (mLastPont != null)
                canvas.drawLine(mDrawPoint.getX(), mDrawPoint.getY(), mLastPont.getX(), mLastPont.getY(), mPaint);
        }
    }
...
//        if (mPoints == null)
//            return;
        for (DrawPoint point : mDrawPoints) {
            mPaint.setColor(Color.BLACK);
            mPaint.setAntiAlias(true);
            //画图的点的集合
            x = point.getX();
            ...
            canvas.drawText(point.getName(), x, y - 10, mPaint);
        }
    }
}
